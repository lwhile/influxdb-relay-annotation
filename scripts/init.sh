...
# Default-Stop:      0 1 6
# Short-Description: Start influxdb-relay at boot time
### END INIT INFO

# If you modify this, please make sure to also edit influxdb-relay.service
...
function start() {
    ...
    if which start-stop-daemon > /dev/null 2>&1; then
        start-stop-daemon --chuid $GROUP:$USER --start --quiet --pidfile $PIDFILE --exec $DAEMON -- -pidfile $PIDFILE -config $CONFIG $DAEMON_OPTS >>$STDOUT 2>>$STDERR &
    else
        su -s /bin/sh -c "nohup $DAEMON -pidfile $PIDFILE -config $CONFIG $DAEMON_OPTS >>$STDOUT 2>>$STDERR &" $USER
    fi
